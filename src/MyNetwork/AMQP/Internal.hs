...
module MyNetwork.AMQP.Internal where
...
import qualified MyNetwork2.Connection as Conn

import MyNetwork.AMQP.Protocol
import MyNetwork.AMQP.Types
import MyNetwork.AMQP.Helpers
import MyNetwork.AMQP.Generated
import MyNetwork.AMQP.ChannelAllocator
...
watchHeartbeats conn timeout connThread = scheduleAtFixedRate rate $ do
  ...
  where
    ...
    checkSendTimeout = doCheck (connLastSent conn) sendTimeout
        -- (putStrLn ">>> watchHeartbeats :: checkSendTimeout" >> writeFrame (connHandle conn) (Frame 0 HeartbeatPayload))
        (do
            putStrLn ">>> watchHeartbeats :: checkSendTimeout"
            withMVar (connWriteLock conn) $ \_ -> writeFrame (connHandle conn) (Frame 0 HeartbeatPayload)
        )
...
writeFrame handle f@(Frame _ch framePayload) = do
    let
        bs = toStrict $ runPut $ put $ f
    putStrLn $ "\nwriteFrame:  bs length = " ++ show (BS.length bs)
    putStrLn $ "writeFrame :: frame type = " ++ show (frameType framePayload)
    -- print bs
    -- putStrLn $ take 2000 $ show bs
    putStrLn "start writing"
    Conn.connectionPut handle bs
    -- bs `deepseq` Conn.connectionPut handle bs
...
writeAssembly' chan (ContentMethod m properties msg) = do
    ...
    putStrLn $ "writeAssembly' :: BL.length msg = " ++ show (BL.length msg)
    putStrLn $ "writeAssembly' :: connMaxFrameSize $ connection chan = " ++ show (connMaxFrameSize $ connection chan)
