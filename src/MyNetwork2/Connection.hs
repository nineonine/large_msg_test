...
import MyNetwork2.Connection.Types
...
connectionPut connection content = withBackend doWrite connection
    where doWrite (ConnectionStream h) = putStrLn "connectionPut :: ConnectionStream" >> B.hPut h content >> hFlush h
          doWrite (ConnectionSocket s) = do
              putStrLn "connectionPut :: ConnectionSocket :: start"
              -- N.sendAll s content
              my_sendAll s content
              -- my_sendAll_lazy s content
              putStrLn "connectionPut :: ConnectionSocket :: done"
          doWrite (ConnectionTLS ctx)  = putStrLn "connectionPut :: ConnectionTLS"    >> TLS.sendData ctx (L.fromChunks [content])


my_sendAll :: Socket      -- ^ Connected socket
           -> ByteString  -- ^ Data to send
           -> IO ()
my_sendAll sock bs = do
    putStrLn $ "my_sendAll :: about to send " ++ show (B.length bs) ++ " bytes"
    sent <- N.send sock bs
    putStrLn $ "my_sendAll :: sent = " ++ show sent
    when (sent < B.length bs) $ my_sendAll sock (B.drop sent bs)


my_sendAll_lazy :: Socket      -- ^ Connected socket
                -> ByteString  -- ^ Data to send
                -> IO ()
my_sendAll_lazy sock bs = do
    putStrLn $ "my_sendAll_lazy :: about to send " ++ show (B.length bs) ++ " bytes"
    NL.sendAll sock $ L.fromStrict bs
    putStrLn $ "my_sendAll_lazy :: done"
